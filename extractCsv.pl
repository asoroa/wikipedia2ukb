...
sub summarise_anchors() {
	...
	open(ANCHOR_SUMMARY, "> $out_dir/anchor_summary.csv") or die "'$out_dir' is not writable/\n" ;
	...
	while ($pass < $passes) {
		...
		my $anchor_fh = &open_maybe_bz2("$out_dir/anchor.csv")
		  or die "cannot find '$out_dir/anchor.csv'. You must run extractCoreTables.pl first\n" ;

		my $start_time = time  ;
		my $parts_total = -s "$out_dir/anchor.csv" ;

		my $anchor_count = 0 ;
		my $parts_done = 0 ;

		while (defined (my $line = <$anchor_fh>)) {
			$parts_done = $parts_done + length $line ;
...
sub summarize_generality() {
	...
	my $start_time = time  ;
	my $parts_total = -s "$out_dir/categorylink.csv" ;
	my $parts_done = 0 ;

	my $categorylink_fh = &open_maybe_bz2("$out_dir/categorylink.csv") ;

	while (defined (my $line = <$categorylink_fh>)) {
		$parts_done = $parts_done + length $line ;
		chomp($line) ;
	...
	open(GENERALITY, "> $out_dir/generality.csv") ;
...
sub summarize_linkcounts() {
	...
	my $start_time = time  ;
	my $parts_total = -s "$out_dir/pagelink.csv" ;
	my $parts_done = 0 ;

	my $pagelink_fh = &open_maybe_bz2("$out_dir/pagelink.csv") ;

	my $lastFrom = 0 ;
	my $lastFrom_out = 0 ;

	while (defined (my $line = <$pagelink_fh>)) {
		$parts_done = $parts_done + length $line ;
		chomp($line) ;
	...
	open(LINKCOUNT, "> $out_dir/linkcount.csv") ;
...
sub summarize_linksOut() {
	...
	my $start_time = time ;
	my $parts_total = -s "$out_dir/linkcount.csv" ;
	my $parts_done = 0 ;

	my $linkcount_fh = &open_maybe_bz2("$out_dir/linkcount.csv") ;

	my $count = 0 ;

	while (defined (my $line = <$linkcount_fh>)) {
		$parts_done = $parts_done + length $line ;
		chomp($line) ;
	...
	$start_time = time ;
	$parts_total = -s "$out_dir/pagelink.csv" ;
	$parts_done = 0 ;

	my $pagelink_fh = &open_maybe_bz2("$out_dir/pagelink.csv") ;
	open(LINKSOUT, "> $out_dir/pagelink_out.csv") ;
...
sub summarize_linksIn() {
	...
	open(LINKSIN, "> $out_dir/pagelink_in.csv") ;
	...
	#pen(LINKSIN, "> $out_dir/pagelink_in.csv") ;
...
sub extractContent() {
	...
	open (CONTENT, "> $out_dir/content.csv") ;
