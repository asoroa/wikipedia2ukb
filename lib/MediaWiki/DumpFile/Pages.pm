...
our $VERSION = '0.2.2';
our $TESTED_SCHEMA_VERSION = '0.10';
...
# returns:
#    -1 $wiki_version < $version
#     0 $wiki_version == $version
#    +1 $wiki_version > $version

sub wiki_version_cmp {
	my ($wiki_version, $version) = @_;

    my @WikiV = split(/\./, $wiki_version);
    my @V = split(/\./, $version);

    my $m = scalar(@WikiV);
    $m = scalar(@V) if scalar(@V) < scalar(@WikiV);
    for(my $i = 0; $i < $m; $i++) {
        my $res = $WikiV[$i] <=> $V[$i];
        return $res if $res;
    }
    return scalar(@WikiV) <=> scalar(@V);
}
...
sub _init_xml {
	...
	if (&wiki_version_cmp($version, "0.2") == 1) {
...
sub _version_enforce {
	...
	if (&wiki_version_cmp($version, $TESTED_SCHEMA_VERSION) == 1) {
		my $filename;
		my $msg;
...
sub new {
    ...
    if (&MediaWiki::DumpFile::Pages::wiki_version_cmp($version, "0.4") != -1) {
